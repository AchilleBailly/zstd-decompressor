...
pub mod huffman {
    ...
    pub enum HuffmanDecoder {
        Absent, 
        Symbol {payload : u8},
        Tree {left : Box<HuffmanDecoder>, right : Box<HuffmanDecoder>}
    ...
    impl Iterator for HuffmanDecoderIterator<'_> {
        ...
        fn next(&mut self ) -> Option<Self::Item> {
            match self.noeuds.pop() {
                Some((HuffmanDecoder::Symbol{payload :symb}, prefixe)) => Some(vec![(HuffmanDecoder::Symbol{payload:*symb}, prefixe)]),
                Some((HuffmanDecoder::Tree {left : gauche, right : droite}, mut prefixe)) => {
                    self.noeuds.push((droite, {prefixe.push('1'); prefixe.clone()}));
                    self.noeuds.push((gauche, {prefixe.push('0'); prefixe}));
                    self.next()
                },
                _ => self.next(),
            }
        ...
        fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), Error>{
            ...
            for noeud in vec![(self, " ")] {
                match noeud {
                    ...
                    (HuffmanDecoder::Symbol{payload : symb}, prefixe) => {t.field( prefixe, symb); ()}
                    (HuffmanDecoder::Tree{left : gauche, right : droite}, prefixe) =>(),
                }
            }
    ...
    impl HuffmanDecoder {
        pub fn insert(&mut self, symbol: u8, width: u8) -> bool {
            if width == 0{
                ...
                match self {
                    HuffmanDecoder::Tree{left : gauche, right : droite} => if gauche.insert(symbol, width -1) != true {droite.insert(symbol, width -1);},
                    HuffmanDecoder::Absent => {*self =  HuffmanDecoder::Tree{left : Box::new(HuffmanDecoder::Absent), right : Box::new(HuffmanDecoder::Absent)};
        ...
        pub fn from_weights (weights : Vec<u8>) -> HuffmanDecoder {
            ...
            let manquant:u8 = ((pow(2,puissance.try_into().unwrap()) - sum).ilog2() + 1).try_into().unwrap();
